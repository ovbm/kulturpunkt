...
(function (d) {
  var config = {
    ...
    h = d.documentElement, t = setTimeout(function () {h.className = h.className.replace(/\bwf-loading\b/g, '') + ' wf-inactive';}, config.scriptTimeout), tk = d.createElement('script'), f = false, s = d.getElementsByTagName('script')[0], a; h.className += ' wf-loading'; tk.src = 'https://use.typekit.net/' + config.kitId + '.js'; tk.async = true; tk.onload = tk.onreadystatechange = function () {
      a = this.readyState; if (f || a && a != 'complete' && a != 'loaded') {return;} f = true; clearTimeout(t); try {Typekit.load(config);}
      catch (e) {}
...
function loadjQuery(e, t) {var n = document.createElement('script'); n.setAttribute('src', e); n.onload = t; n.onreadystatechange = function () {if (this.readyState == 'complete' || this.readyState == 'loaded') {t();}}; document.getElementsByTagName('head')[0].appendChild(n);} function main() {
  var $cr = jQuery.noConflict(); var old_src; $cr(document).ready(function () {
    $cr('.cr_form').submit(function () {
      $cr(this).find('.clever_form_error').removeClass('clever_form_error'); $cr(this).find('.clever_form_note').remove(); $cr(this).find('.musthave').find('input, textarea').each(function () {
        if (jQuery.trim($cr(this).val()) == '' || ($cr(this).is(':checkbox')) || ($cr(this).is(':radio'))) {
          if ($cr(this).is(':checkbox') || ($cr(this).is(':radio'))) {if (!$cr(this).parent().find(':checked').is(':checked')) {$cr(this).parent().addClass('clever_form_error');}}
          else {$cr(this).addClass('clever_form_error');}
        }
      }); if ($cr(this).attr('action').search(document.domain) > 0 && $cr('.cr_form').attr('action').search('wcs') > 0) {var cr_email = $cr(this).find('input[name=email]'); var unsub = false; if ($cr("input['name=cr_subunsubscribe'][value='false']").length) {if ($cr("input['name=cr_subunsubscribe'][value='false']").is(':checked')) {unsub = true;}} if (cr_email.val() && !unsub) {$cr.ajax({type: 'GET', url: $cr('.cr_form').attr('action').replace('wcs', 'check_email') + $cr(this).find('input[name=email]').val(), success: function (data) {if (data) {cr_email.addClass('clever_form_error').before("<div class='clever_form_note cr_font'>" + data + '</div>'); return false;}}, async: false});} var cr_captcha = $cr(this).find('input[name=captcha]'); if (cr_captcha.val()) {$cr.ajax({type: 'GET', url: $cr('.cr_form').attr('action').replace('wcs', 'check_captcha') + $cr(this).find('input[name=captcha]').val(), success: function (data) {if (data) {cr_captcha.addClass('clever_form_error').after("<div style='display:block' class='clever_form_note cr_font'>" + data + '</div>'); return false;}}, async: false});}} if ($cr(this).find('.clever_form_error').length) {return false;} return true;
  ...
  }); function captcha_reload() {var timestamp = new Date().getTime(); $cr("div[rel='captcha'] img:not(.captcha2_reload)").attr('src', ''); $cr("div[rel='captcha'] img:not(.captcha2_reload)").attr('src', old_src + '?t=' + timestamp); return false;}

}
if (typeof jQuery === 'undefined') {loadjQuery('//ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js', main);}
else {main();}
